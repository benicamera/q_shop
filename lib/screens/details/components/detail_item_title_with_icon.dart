...
class DetailItemTitleWithIcon extends StatelessWidget {
  ...
  const DetailItemTitleWithIcon({Key key, @required this.product, this.size})
      : super(key: key);
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Row(
            children: <Widget>[
              Padding(
                  padding: EdgeInsetsDirectional.only(start: size.width / 2.8),
                  child: Hero(
                    tag: (product.name + "1"),
                    child: Icon(
                      (wasCreated())
  ...
  bool wasCreated() {
    for (int i = 0; i < DefaultPrefs.defProducts.length; i++) {
      if (DefaultPrefs.defProducts[i].name == product.name) {
        return false;
      }
    }
    return true;
