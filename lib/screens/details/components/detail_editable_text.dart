...
class DetailEditableText extends StatefulWidget {
  ...
  const DetailEditableText({Key key, this.product, this.index}) : super(key: key);
  @override
  _DetailEditableTextState createState() => _DetailEditableTextState(product: this.product, index: this.index);
}
...
class _DetailEditableTextState extends State<DetailEditableText> {
  ...
  _DetailEditableTextState({this.product, this.index}); //TODO: get note
  ...
  int getProdIndex(ShopList currList){
    ...
    for(int i = 0; i < prodList.length; i++){
      if(prodList[i].name == product.name)
        return i;
    }
    return null;
  }
